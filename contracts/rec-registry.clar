...
(define-trait rec-registry-trait
  (
    (get-rec (uint) (response (tuple (producer principal) (energy-amount uint) (production-date uint) (expiration-date uint) (status (string-ascii 20))) uint))
    (update-rec-status (uint (string-ascii 20)) (response bool uint))
  )
)
